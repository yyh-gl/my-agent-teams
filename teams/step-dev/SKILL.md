---
name: step-dev-team
description: step-devチームを起動するスキル。Lead・Planner・Implementer・Tester・Reviewerの5 agentが協調して機能実装・テスト・レビューを実行する。Plannerが作成した実装プランをレイヤー（フェーズ）ごとに分割し、各レイヤーの実装・テスト・レビュー完了後に必ず人の確認を挟んでから次のレイヤーへ進む。「step-devチームで実装して」「ステップごとに確認しながら実装して」「レイヤーごとに実装して」などの依頼が来たら必ずこのスキルを使用すること。
---
...
Agent ツールで各メンバーを起動する。**`team_name: "step-dev-team"` と `name` パラメータを必ず指定する**（指定しないとチームに参加できない）:

- Planner（**必須。全タスクで起動する**）: `subagent_type: "planner"`, `name: "Planner"`, `team_name: "step-dev-team"`
- Implementer: `subagent_type: "implementer"`, `name: "Implementer"`, `team_name: "step-dev-team"`
- Tester: `subagent_type: "tester"`, `name: "Tester"`, `team_name: "step-dev-team"`
- Reviewer: `subagent_type: "reviewer"`, `name: "Reviewer"`, `team_name: "step-dev-team"`
...
1. Planner のプラン完了後、Lead が実装レイヤー一覧を抽出し `AskUserQuestion` でユーザーに提示・確認する
2. 各レイヤーについて: Implementer → Tester & Reviewer（並行）→ 品質ゲート判定
3. **各レイヤー完了後、必ず `AskUserQuestion` で人の確認を取ってから次のレイヤーへ進む**
4. 全レイヤー完了まで繰り返す
